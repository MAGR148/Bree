...
export default class extends Controller {
  ...
  select(target) {
    this.resultsTarget.querySelectorAll('[aria-selected="true"]').forEach((el) => {
  ...
  identifyOptions() {
    ...
    this.resultsTarget.querySelectorAll('[role="option"]:not([id])').forEach((el) => {
      el.id = `${this.resultsTarget.id}-option-${id += 1}`;
    });
  }
